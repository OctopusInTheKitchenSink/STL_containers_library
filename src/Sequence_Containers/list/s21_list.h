...
class list : public Vector_list<T> {
 ...
 private:
  ...
  class ListIterator {
   ...
   public:
    ...
    Node *get_prev() { return current->prev; };

    Node *get_next() { return current->next; };

    Node *get_current() { return current; }
  };

  ListIterator start() const { return ListIterator(head); };

  ListIterator finish() const { return ListIterator(tail); };
 ...
 public:
  ...
  size_type node_size() { return sizeof(Node); };

 public:
  const_reference get_data(Node *ptr) { return ptr.data; };
  ...
  iterator end_iter() const { return nullptr; }
  ...
  void erase(iterator pos) {
    if (!pos.get_current() || empty())
      throw std::out_of_range("You try to inserte node without addres");
    if (pos == end()) {
  ...
  bool check_iterator(const_iterator pos) const {
    if (pos == nullptr)
      throw std::invalid_argument(
          "You try to splice other list into nullptr place");
    bool flag = false;
    ...
    if (!flag)
      throw std::invalid_argument("Pos is not in the range [begin(), end())");
    return flag;
  };
