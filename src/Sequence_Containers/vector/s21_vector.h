...
namespace s21{
    ...
    public:
        ...
        reference at(size_type pos){
            if(pos >= capacity_ || pos < 0) throw std::out_of_range("You try to access to element out of range");
            return vec[pos];
        };
        ...
        std::pair<bool, size_type> check_pos(iterator &pos){
            if(pos.get_curr() < begin().get_curr() || pos.get_curr() > &vec[capacity_ - 1]) return {false, 0};
        ...
        iterator insert(iterator pos, const_reference value){
            ...
            if(!res.first) throw std::out_of_range("Try to access element out of range");
        ...
        void erase(iterator pos){
            std::pair<bool, size_type> res = check_pos(pos);
            if(!res.first || !size()) throw std::out_of_range("Try to access element out of range");
            T* tmp = pos.get_curr();
            while(pos != end()){
                *tmp = *(++pos);
                ++tmp;
